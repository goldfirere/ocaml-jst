...
let rec estimate_type_layout env ty =
  ...
  | Tconstr(p, _, _) -> begin
      ...
      | exception Not_found -> Layout (any ~creation:(Missing_cmi p))
    end
  | Tvariant row ->
...
let rec constrain_type_layout ~fixed env ty layout fuel =
  let sub l1 l2 = Result.map ignore (Layout.sub l1 l2) in
  let constrain_unboxed ty =
    match estimate_type_layout env ty with
    | Layout ty_layout -> sub ty_layout layout
    | TyVar (ty_layout, ty) ->
      if fixed then sub ty_layout layout
      else
        let layout_inter =
  ...
  | Tconstr(p, _args, _abbrev) -> begin
      let layout_bound =
        ...
        | exception Not_found ->
          Layout.any ~creation:(Missing_cmi p)
        end
      in
      match sub layout_bound layout with
      | Ok () as ok -> ok
