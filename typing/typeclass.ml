...
type error =
  ...
  | Non_value_binding of string * Kkind.Violation.t
  ...
  | Pctf_val ({txt=lab}, mut, virt, sty) ->
      mkctf_with_attrs
        (fun () ->
          ...
          let ty = cty.ctyp_type in
          begin match Ctype.constrain_type_kkind env ty Kkind.value with
          | Ok _ -> ()
      ...
      mkctf_with_attrs
        (fun () ->
           ...
           | Ptyp_poly ([],sty'), Public ->
               let expected_ty = Ctype.newvar Kkind.value in
  ...
  | Pcf_val (label, mut, Cfk_virtual styp) ->
      with_attrs
        (fun () ->
           ...
           begin
             match Ctype.constrain_type_kkind val_env ty Kkind.value with
             | Ok _ -> ()
  ...
  | Pcf_val (label, mut, Cfk_concrete (override, sdefinition)) ->
      with_attrs
        (fun () ->
           ...
           begin
             match Ctype.constrain_type_kkind val_env definition.exp_type
                     Kkind.value with
             | Ok _ -> ()
             | Error err -> raise (Error(label.loc, val_env,
                                         Non_value_binding(label.txt, err)))
        ...
        (fun () ->
           ...
           begin
             try
               ...
               | Tvar _ ->
                   let ty' = Ctype.newvar Kkind.value in
...
let var_option = Predef.type_option (Btype.newgenvar Kkind.value)
...
let temp_abbrev loc env id arity uid =
  ...
  for _i = 1 to arity do
    params := Ctype.newvar Kkind.value :: !params
  done;
  let ty = Ctype.newobj (Ctype.newvar Kkind.value) in
  let env =
...
let class_infos define_class kind
  ...
  let ci_params =
    let make_param (sty, v) =
      try
          (transl_type_param env sty Kkind.value, v)
      with Already_bound ->
...
let report_error env ppf = function
  ...
  | Non_value_binding (nm, err) ->
    fprintf ppf
      ...
      (Kkind.Violation.report_with_name ~name:nm) err
