...
type error =
  ...
  | Not_a_value of Kkind.Violation.t * type_forcing_context option
      ...
      | ({pv_id = x1; pv_type = t1; pv_mode = m1; _} as pv1)::rem1,
        ...
        when Ident.equal x1 x2 ->
          if x1==x2 then
            ...
            begin try
              unify_var env (newvar Kkind.any) t1;
              unify env t1 t2
            with
            | Unify err ->
...
and build_as_type_aux ~refine (env : Env.t ref) p =
  ...
  | Tpat_variant(l, p', _) ->
      ...
      let ty =
        ...
        newty (Tvariant (create_row ~fields ~more:(newvar Kkind.value)
      ...
      | Some row ->
          ...
          let ty =
            newty (Tvariant (create_row ~fields ~fixed ~name
                               ~closed:false ~more:(newvar Kkind.value)))
          in
          ty, mode
      end
  | Tpat_constant _ ->
...
let solve_Ppat_tuple (type a) ~refine ~alloc_mode loc env (args : a list) expected_ty =
  ...
  let ann =
    List.map2
      (fun p mode -> (p, newgenvar Kkind.value, simple_pat_mode mode))
      args arg_modes
  in
...
let solve_constructor_annotation env name_list sty ty_args ty_ex =
  ...
  let ids =
    List.map
      (fun name ->
         (* CJC XXX I expect this needs to change when we allow kkind
            annotations on explicitly quantified vars in gadt constructors.
            See: https://github.com/ocaml/ocaml/pull/9584/
         *)
        let decl = new_local_type ~loc:name.loc Kkind.value in
        let (id, new_env) =
...
let solve_Ppat_array ~refine loc env expected_ty =
  (* CR ccasinghino: in the future we'll have arrays of other kkinds *)
  let ty_elt = newgenvar Kkind.value in
  let expected_ty = generic_instance expected_ty in
...
let solve_Ppat_lazy  ~refine loc env expected_ty =
  let nv = newgenvar Kkind.value in
...
let solve_Ppat_variant ~refine loc env tag no_arg expected_ty =
  ...
  (arg_type, make_row (newvar Kkind.value), instance expected_ty)

(* Building the or-pattern corresponding to a polymorphic variant type *)
let build_or_pat env loc mode lid =
  let path, decl = Env.lookup_type ~loc:lid.loc lid.txt env in
  let tyl = List.map (fun _ -> newvar Kkind.value) decl.type_params in
  let row0 =
  ...
  let ty = newty (Tvariant (make_row (newvar Kkind.value))) in
  let gloc = {loc with Location.loc_ghost=true} in
  let row' = ref (make_row (newvar Kkind.value)) in
  let pats =
    List.map
      (fun (l,p) ->
...
and type_pat_aux
  ...
  | Ppat_record(lid_sp_list, closed) ->
      ...
      let expected_type, record_ty =
        ...
        | Maybe_a_record_type -> None, newvar Kkind.value
        | Not_a_record_type ->
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  if is_optional l then
    unify_pat (ref val_env) pat (type_option (newvar Kkind.value));
  ...
  let rec loop ty_fun mode_fun rev_args sargs =
    ...
    | (lbl, sarg) :: rest ->
        let (mode_arg, ty_arg_mono, mode_res, ty_res) =
          ...
          | Tvar _ ->
              let ty_arg_mono = newvar Kkind.value in
              let ty_arg = newmono ty_arg_mono in
              let ty_res = newvar Kkind.value in
              if ret_tvar &&
...
let rec is_nonexpansive exp =
  ...
  | Texp_sequence (_e1, _kkind, e2) -> is_nonexpansive e2  (* PR#4354 *)
...
let rec approx_type env sty =
  ...
  | Ptyp_arrow (p, ({ ptyp_desc = Ptyp_poly _ } as arg_sty), sty) ->
      if is_optional p then newvar Kkind.value
      else begin
  ...
  | Ptyp_constr (lid, ctl) ->
      ...
      if List.length ctl <> decl.type_arity then newvar Kkind.any
      else begin
        let tyl = List.map (approx_type env) ctl in
        newconstr path tyl
      end
  | _ -> newvar Kkind.value
...
and type_approx_aux env sexp in_function ty_expected =
  ...
  | Pexp_tuple l ->
      let tys = List.map (fun _ -> newvar Kkind.value) l in
    ...
    match get_desc pty with
      Tpoly (body, tl) ->
        ...
        List.iter2 (fun uvar var ->
          (* This checks that the term doesn't require more specific kkinds
          ...
          (* CJC XXX expand_head here is needed for examples like:
             ...
             need to expand the var to find the variable with the kkind we want
             to check.

             However, I should come back and think about his more carefully:
             1) [polyfy], which is called below, also does this expansion.
                It would be nice to just move the kkind check there,
          ...
          *)
          ...
          | Tvar { kkind = kkind2; _ } -> begin
              match check_type_kkind env uvar kkind2 with
              | Ok _ -> ()
              | Error err ->
                error exp_ty ty_expected
                  [Errortrace.Bad_kkind (uvar,err)]
            end
...
let check_absent_variant env =
  iter_general_pattern { f = fun (type k) (pat : k general_pattern) ->
    ...
    | Tpat_variant (s, arg, row) ->
      ...
      let row' =
        create_row ~fields
          ~more:(newvar Kkind.value) ~closed:false ~fixed:None ~name:None
      in
      (* Should fail *)
...
and type_expect_
        ...
        if TypeSet.mem ty seen then false else
          match get_desc ty with
            Tarrow (_l, ty_arg, ty_fun, _com) ->
              (try unify_var env (newvar Kkind.any) ty_arg
               with Unify _ -> assert false);
              ret_tvar (TypeSet.add ty seen) ty_fun
          | Tvar _ ->
              let v = newvar Kkind.any in
  ...
  | Pexp_tuple sexpl ->
      ...
      assert (arity >= 2);
      register_allocation expected_mode;
      let subtypes = List.map (fun _ -> newgenvar Kkind.value) sexpl in
  ...
  | Pexp_array(sargl) ->
      register_allocation expected_mode;
      let ty = newgenvar Kkind.value in
  ...
  | Pexp_sequence(sexp1, sexp2) ->
      ...
      let kkind = type_kkind env exp1.exp_type in
      re {
        exp_desc = Texp_sequence(exp1, kkind, exp2);
        exp_loc = loc; exp_extra = [];
  ...
  | Pexp_while(scond, sbody) ->
      ...
      in
      let wh_body_kkind = Ctype.type_kkind env wh_body.exp_type in
      rue {
        exp_desc =
  ...
  | Pexp_for(param, slow, shigh, dir, sbody) ->
      ...
      in
      let for_body_kkind = Ctype.type_kkind env for_body.exp_type in
      rue {
        exp_desc = Texp_for {for_id; for_pat = param; for_from; for_to;
                             for_dir = dir; for_body; for_body_kkind;
                             for_region};
        exp_loc = loc; exp_extra = [];
        exp_type = instance Predef.type_unit;
        exp_mode = expected_mode.mode;
  ...
  | Pexp_newtype({txt=name}, sbody) ->
      let kkind =
        Kkind.of_const_option ~default:Kkind.value
          (Builtin_attributes.kkind sexp.pexp_attributes)
      in
      let ty =
      ...
      let decl = new_local_type ~loc kkind in
      let scope = create_scope () in
  ...
  | Pexp_letop{ let_ = slet; ands = sands; body = sbody } ->
      ...
      let spat_params, ty_params =
        loop slet.pbop_pat (newvar Kkind.value) sands
      in
      let ty_func_result = newvar Kkind.value in
      ...
      in
      let ty_result = newvar Kkind.value in
      let ty_andops = newvar Kkind.value in
      let ty_op =
...
and type_function ?in_function loc attrs env (expected_mode : expected_mode)
  ...
  if not has_poly && not (tpoly_is_mono ty_arg) && !Clflags.principal
    ...
    let really_poly =
      try
        unify env (newmono (newvar Kkind.any)) ty_arg;
        false
      with Unify _ -> true
    in
    Btype.backtrack snap;
...
and type_apply_arg env ~app_loc ~funct ~index ~position ~partial_app (lbl, arg) =
  ...
  | Arg (Unknown_arg { sarg; ty_arg_mono; mode_arg }) ->
      ...
      if is_optional lbl then
        unify_exp env arg (type_option(newvar Kkind.value));
      (lbl, Arg arg)
...
and type_statement ?explanation env sexp =
  ...
  end_def();
  let ty = expand_head env exp.exp_type and tv = newvar Kkind.any in
...
(*  Format.printf "@[%i %i@ %a@]@." lev (get_current_level())
  ...
  let ty_arg' = newvar Kkind.any in
  let unify_pats ty =
  ...
  List.iter (fun { pat_vars; _ } ->
    iter_pattern_variables_type
      (fun t -> unify_var env (newvar Kkind.any) t) pat_vars
  ) half_typed_cases;
  end_def ();
  generalize ty_arg';
  List.iter (fun { pat_vars; _ } ->
  ...
  if may_contain_gadts then begin
    ...
    unify_exp_types loc env (instance ty_res) (newvar Kkind.any) ;
  end;
  cases, partial
...
and type_andops env sarg sands expected_ty =
  let rec loop env let_sarg rev_sands expected_ty =
    ...
    | { pbop_op = sop; pbop_exp = sexp; pbop_loc = loc; _ } :: rest ->
        ...
        let ty_arg = newvar Kkind.value in
        let ty_rest = newvar Kkind.value in
        let ty_result = newvar Kkind.value in
        let arrow_desc = (Nolabel,Alloc_mode.global,Alloc_mode.global) in
        let ty_rest_fun =
...
| Extensions.Eexp_arr_comprehension (sbody, comp_typell) ->
  ...
  and type_comprehension ~loc ~env ~ty_expected ~(expected_mode : expected_mode)
    ...
    if !Clflags.principal then begin_def ();
    let without_arr_ty = Ctype.newvar Kkind.value in
  ...
  and type_comprehension_clause ~body_env ~env ~loc ~container_type
    ...
    | In (param, siter) ->
      let item_ty = newvar Kkind.value in
  ...
  | Not_a_value (err, explanation) ->
    Location.error_of_printer ~loc (fun ppf () ->
      fprintf ppf "Method types must have layout value.@ %a"
        (Kkind.Violation.report_with_name ~name:"This expression")
        err;
      report_type_expected_explanation_opt explanation ppf)
      ()
