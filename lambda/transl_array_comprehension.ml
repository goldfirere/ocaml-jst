...
module Precompute_array_size : sig
  ...
  let safe_product_pos_vals ~loc = function
    ...
    | x :: xs -> List.fold_left (safe_mul_pos_vals ~loc) x.Let_binding.occur xs
    | []      -> int 1
  ...
  module Fixed_size_array = struct
    ...
    let is_empty ~loc t =
      ...
      | Range { start; stop; direction} -> begin
          let start = start.occur in
          let stop  = stop.occur  in
    ...
    let size_nonempty ~loc = function
      | Range { start; stop; direction } ->
          ...
          Let_binding.let_one range_size
            ...
            (Lifthenelse(range_size.occur > l0,
                         range_size.occur,
...
let iterator ~transl_exp ~scopes ~loc
  ...
  | Texp_comp_in { pattern; sequence = iter_arr_exp } ->
      ...
      let iter_len =
        ...
        Let_binding.make (Immutable Alias) Pintval
          "iter_len"
          (Lprim(Parraylength iter_arr_kind, [iter_arr.occur], loc))
      in
...
let initial_array ~loc ~array_kind ~array_size ~array_sizing =
  ...
  let array_let_kind, array_value =
    ...
    | Fixed_size, (Pintarray | Paddrarray) ->
        Immutable StrictOpt,
        make_vect ~loc ~length:array_size.occur ~init:(int 0)
    | Fixed_size, Pfloatarray ->
        Immutable StrictOpt, make_float_vect ~loc array_size.occur
...
let body
  ...
  *)
  ...
  let set_element_raw elt =
    (* array.(index) <- elt *)
    Lprim(Parraysetu array_kind, [array.occur; index.occur; elt], loc)
  ...
  let set_element_in_bounds elt = match array_sizing with
    ...
    | Dynamic_size ->
        Lsequence(
          ...
          Lifthenelse(index.occur < array_size.occur,
            lambda_unit,
            Lsequence(
              Lassign(array_size.id, i 2 * array_size.occur),
              Lassign(array.id,
                      Resizable_array.double ~loc array.occur)),
            Pintval (* [unit] is immediate *)),
  ...
  Lsequence(
    ...
    Lassign(index.id, index.occur + l1))
  ...
  let comprehension =
    Let_binding.let_all
      ...
      (Lsequence(
         ...
         | Fixed_size -> array.occur
         | Dynamic_size ->
             array_sub ~loc array.occur ~offset:(int 0)
               ~length:index.occur))
  in
