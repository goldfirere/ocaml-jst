  ...
  let bindings, for_ =
    ...
    | In (pat , e2) ->
        ...
        let for_ = Lfor {
          ...
          for_dir = Upto;
          for_body =
            Matching.for_let ~scopes pat.pat_loc Not_void
              (Lprim(Parrayrefu(in_kind),
                     [Lvar(in_var); Lvar(index)], loc))
              Kkind.Layout.value pat (valuekind_of_arraykind in_kind) body;
          for_region = true
        }
        in
...
let transl_list_comp type_comp body acc_var mats ~transl_exp ~scopes ~loc =
  ...
  let param, pval, args, func, body, mats =
    ...
    | In (pat, in_) ->
      ...
      let body =
        Matching.for_let ~scopes pat.pat_loc Not_void (Lvar(pat_id))
          Kkind.Layout.value pat pval body
      in
